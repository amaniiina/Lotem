...
export default class CreateLocGroupForm extends Component {
  constructor(props) {
    super(props)
    this.userid = firebase.auth().currentUser.uid
    this.groupKey = 'mainGroup'
  ...
  createGroup() {
    let newkey = this.state.groupName
    let ref = firebase.firestore().collection('Groups').doc(newkey)
        ...
        <Content>
          <View style={{ margin: 20, flexDirection: "row", alignItems: 'center' }}>
            ...
            <View style={{ height: 80, flexDirection: 'column' }}>
              <TextInput
                maxLength={30}
                placeholder='type Location group name...'
                onChangeText={(groupName) => this.setState({ groupName })}
                style={
                  styles.input
                }
              />
              <Text style={styles.txt}>Provide a Location name and optional group icon</Text>
            </View>
          </View>
          <Button
